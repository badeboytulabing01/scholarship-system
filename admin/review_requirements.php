...
<?php $view_query = mysqli_query($conn, "SELECT * FROM tbl_student_info LEFT JOIN  tbl_requirements ON tbl_student_info.student_id = tbl_requirements.student_id WHERE tbl_requirements.submit_status='pending' limit 10"); ?>
  ...
  $submit_status      = $row["submit_status"];
       ...
       <tr>
         ...
         <input type="hidden" name="student_id" value="<?php echo $row['student_id'];?>">
          ...
          <form action="failed_email.php" method="POST">
          <input type="hidden" name="student_id" value="<?php echo $row['student_id'];?>">
          <input type="hidden" name="email"  value="<?php echo $row['s_email'];?>">
          <input type="hidden" name="subject" value="CVSU scholarship">
